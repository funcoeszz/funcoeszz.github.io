...
# ----------------------------------------------------------------------------
zzzz(){ 
local pat vl vr URL='http://verde666.org/zz' cor='não'; [ -f "$ZZPATH" ] &&
...
if [ "$1" = '-z' -o "$1" = '-h' -o "$1" = '--help' ]; then # -h)zzzz -z)resto
  ...
  vr=`sed '/atual/!d;s/.*: //' $ZZTMP`; echozz $vr ; echo
...
elif [ "$1" = '--bashrc' ]; then local cfg="source $ZZPATH" arq=~/.bashrc
  if ! grep -q "^ *$cfg" $arq; then echo "$cfg" >> $arq; echo 'feito!'
  else echo "as funções já estão no $arq!"; fi
else [ "$ZZC" = '1' ] && cor='sim' # mostra informações sobre as funçÕes
  echozz "( local)\c"; echo " $ZZPATH"; echozz "(versão)\c"; echo " $vl"
  echozz "( cores)\c"; echo " $cor"; echozz "(   tmp)\c"; echo -e " $ZZTMP\n"
