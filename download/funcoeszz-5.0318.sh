...
if [ "$1" = '-z' -o "$1" = '-h' -o "$1" = '--help' ]; then # -h)zzzz -z)resto
  ...
  local pat="Uso: [^ ]*${3:-zzzz}\b"; zzajuda | grep -C9 "^$pat" | sed ":a
  ;$ bz;N;ba;:z;s/.*\n---*\(\n\)\(.*$pat\)/\1\2/;s/\(\n\)---.*/\1/"; return 0
fi
...
for tipo in quina megasena duplasena lotomania lotofacil; do echozz $tipo:
 ...
 sed -n '/\([0-9][0-9] \{1,3\}- \{1,3\}\)\{4\}/p;/Concurso n/p' |
 sed ':a;$!N;s/\n/@@/;ta ; s/n.mero // ; s/^\( *Con.*)\)@@\(.*\)/\2@@\1/;
...
sed -n 's,^[^/]*/\([0-9A-F]\+\) .*,\1,p' | while read id; do
...
[ "$s" != "${s#*p}" ] && { URL='http://pontobr.org'
  echo ; echozz "* Ponto BR ($URL):"; $ZZWWWHTML $URL |
...
sed -n '/-- begin insert/,/-- end insert/{ //d; /<\/\?\(ul\|li\)>/d;s/^[\t ]*//
s/.*="current_time_in_\([^.]*\).aspx">\(.*\)<.*/\1 \2/;s/<[^>]*>//g;p;}'>$arq;}
