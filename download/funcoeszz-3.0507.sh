...
# VEJA TAMBÉM: O pacote de funções do Thobias, similar às funções ZZ
...
echozz(){
 [ "$ZZC" != 1 ] && echo -e $* || { echo -e "\033[1m$*"; echo -ne "\033[m"; }
}
seqzz(){
...
# ----------------------------------------------------------------------------
# encontra o pacote que tem um arquivo ou biblioteca qualquer (demora)
...
# OBS.: funciona para os anos de 2001 e 2002
# uso: zzirpf ano número-cpf
# ex.: zzirpf 2002 123.456.789-69
# ----------------------------------------------------------------------------
...
local URL=http://www.ibiblio.org/pub/Linux/docs/HOWTO/ arq=$ZZTMP.howto z=$1
[ "$z" = '--atualiza' ] && { rm -f $arq ; z='' ; } # prepara flags
[ -s "$arq" ] || { echo -n 'AGUARDE. Atualizando listagem...'
  $ZZWWWDUMP $URL | sed '/-HOWTO /!d;s/.* \([^ ]*\)-HOWTO .*/\1/;' > $arq
  echo ' feito!' ; }
[ "$z" ] && grep -i "$1" $arq | sed "s,.*,$URL&-HOWTO,"
}
...
[ "$s" != "${s#*z}" ] && { URL='http://brlinux.linuxsecurity.com.br/noticias/'
  ...
  sed -n 's/^ *//;/^Postado por/{x;p;};h' | $limite; }
...
if [ "$1" ]; then
  if [ "$1" = '--help' -o "$1" = '-h' ]; then $0
    ...
    type "zz${1#zz}" >&- 2>&- || { # a função existe?
    echo "ERRO: a função 'zz${1#zz}' não existe! (tente --help)"; exit 1; }
    "zz${@#zz}"                    # vai!
  fi
