...
#      $ZZEXTRADIR - Caminho completo para o diretório com funções adicionais
...
ZZEXTRADIR_DFT="$HOME/zzextra"    # rota absoluta do diretório de extras
...
[ "$ZZEXTRADIR" ] || ZZEXTRADIR=$ZZEXTRADIR_DFT
[ -f "$ZZEXTRA" ] || ZZEXTRA=
[ -d "$ZZEXTRADIR" ] || ZZEXTRADIR=
#
### Últimos ajustes
#
ZZCOR="${ZZCOR:-$ZZCOR_DFT}"
ZZTMP="${ZZTMPDIR:-$ZZTMPDIR_DFT}/zz"
unset ZZCOR_DFT ZZPATH_DFT ZZEXTRA_DFT ZZEXTRADIR_DFT ZZTMPDIR_DFT
#
#
##############################################################################
...
zzajuda ()
{
	...
	local zzcor_pager
	local tmp="$ZZTMP.ajuda.$$"

	# Compõe um único bloco de texto com todas as funções (padrão e extra)
	(
		cat "$ZZPATH"
		echo "# ------------------------------"
		echo "# #### E X T R A"
		echo "# ------------------------------"
		cat "${ZZEXTRA}" "${ZZEXTRADIR:-XXvazioXX}"/zz* 2>/dev/null
	) |
		# Formata o bloco deixando apenas os cabeçalhos das funções
		sed '
			1 {
	...
	# Já foram coletadas as informações sobre todas as funções, vamos ao processamento

	case "$1" in
		--uso)
			# Lista com sintaxe de uso, basta pescar as linhas Uso:
			sed -n 's/^Uso: zz/zz/p' "$tmp" |
				sort |
				zztool acha '^zz[^ ]*'
		;;
		--lista)
			...
			sed -n '
				# Primeiro extraímos a descrição da função,
				# que é a primeira linha após o -----------
				#
				/^----*$/ {
					...
					# Ignora início de seção
					/^####/	d

					# Ignora a linha com a URL do site (se tiver)
					/^http:/ n

					# Remove ponto final da frase
					s/\.$//

					# Guarda a descrição para usar depois
					h
				}

				# O nome da função é extraído da linha Uso:
				/^Uso: / {
					# Isola o nome da função
					s///
					s/ .*//
					...
					# Junta a descrição (guardada previamente)
					G
					s/\n/ /p
				}' "$tmp" |
				sort |
...
zzzz ()
{
	...
	case "$1" in
		...
		*)
			...
			zztool acha '^[^)]*)' "(extras) ${ZZEXTRA}${ZZEXTRA:+ }${ZZEXTRADIR}"
			zztool acha '^[^)]*)' "(  site) $url_site"
			
			# Lista de todas as funções
			echo
			zztool eco "(( funções disponíveis ))" # Nota: zzzz --tcshrc procura por " fu"
			set |
				# Seds mágicos para extrair e formatar os nomes de funções,
...
# Diretório com funções
if test -n "$ZZEXTRADIR" -a -d "$ZZEXTRADIR"
then
	for arquivo in "${ZZEXTRADIR%/}"/*
	do
 		test -f "$arquivo" && source "$arquivo"
	done
fi
