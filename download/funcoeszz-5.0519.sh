...
for tipo in quina megasena duplasena lotomania lotofacil; do echozz $tipo:
 ...
 s/@@ *ACUM/    ACUM/; s/\.\([0-9]\)00\.000,00\./.\1 Mi/
...
[ "$s" != "${s#*b}" ] && { URL='http://br-linux.org/main/index.xml'
...
[ "$s" != "${s#*u}" ] && { URL='http://www.underlinux.com.br'
  ...
  sed -n '1,/<\/image>/d;/<title>/s/<[^>]*>//gp' | $limite; }
...
local p L='ptg' I=' fre ger jap ita heb spa dut ptg '
[ "$1" ] || { echo -e "zzdicbabylon [idioma] palavra\n idiomas:$I" && return;}
[ "${I% $1 *}" != "$I" ] && L=$1 && shift
$ZZWWWDUMP "http://online.babylon.com/cgi-bin/trans.cgi?lang=$L&word=$1" |
sed 's/^ *// ; /^\[sep-babylon.gif]/,/^\[/!d ; s/^/  /' | sed '1,2d;$d'
}
