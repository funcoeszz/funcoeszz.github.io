...
if [ "$1" = '--atualiza' ]; then # obtém versão nova, se !=, download
  ...
  echozz "( local)\c"; echo " $ZZPATH";
  echozz "(versão)\c"; echo " $vl"
  echozz "( cores)\c"; echo " $cor";
  echozz "(   tmp)\c"; echo " $ZZTMP"
  echozz "(bashrc)\c"; echo " $rc"
  echozz "(extras)\c"; echo " ${ZZEXTRA:-nenhum}"
  echozz "(  site)\c"; echo " $URL"
...
local A A1 A2 D i f_R f_D f_N;
while [ "${1#-}" != "$1" ]; do case "$1" in
  -d) f_D=1;; -r) f_R=1;; -n) f_N=1;; *) break;;
esac; shift; done
[ "$1" ] || { echo 'uso: zzarrumanome [-n] [-d] [-r] arquivo(s)'; return; }
for A in "$@"; do [ "$A" != / ] && A=${A%/}
  [ -f "$A" -o -d "$A" ] || continue; [ -d "$A" ] && { # dir
    [ "${f_R:-0}" -eq 1 ] && zzarrumanome -r ${f_D:+-d} ${f_N:+-n} "$A"/*
    [ "${f_D:-0}" -ne 1 ] && continue; }
  A1="${A##*/}"; D='.'; [ "${A%/*}" != "$A" ] && D="${A%/*}";
  A2=`echo $A1 | sed "s/[\"']//g"'
  ...
  [ $? -ne 0 ] && { echo "Ops. Problemas com UTF-8 (leia $ZZFAQ)"; return; }
  [ "$A1" = "$A2" ] && continue ; [ -f "$D/$A2" -o -d "$D/$A2" ] && {
    i=1 ; while [ -f "$D/$A2.$i" -o -d "$D/$A2.$i" ]; do i=$((i+1)); done
    A2="$A2.$i"; }; #autonumber
  if [ "$f_N" ]; then echo "[-n] $A -> $D/$A2"; else mv -v -- "$A" "$D/$A2"; fi
done
}
...
for arq in "$@"; do
 ...
 [ "$nao" != 1 ] && {
  [ -e "$novo" ] && { echo "Arquivo $novo já existe. Abortando."; return; }
  mv "$arq" "$novo"; }
done
}
...
local dom="${1#www.}" URL='http://registro.br/cgi-bin/whois/'
...
$ZZWWWDUMP $URL | sed -n "s/.* \($ZZERIP\).*/IP: \1/p" | sed 1q
}
...
[ "$s" != "${s#*y}" ] && { URL='http://br.news.yahoo.com/tecnologia/linux'
  echo ; echozz "* Yahoo Linux ($URL):"; $ZZWWWHTML $URL |
  sed -n '/^      <a href/s/<[^>]*>//gp'| sed 's/^[[:blank:]]*//'| $limite; }
...
[ "$d" != "${d#*mandriva}" ] && { echo; echozz "** Atualizações Mandriva"
...
/g' | sed -n 's|^><a href="\([^"]*\)" class=l>\(.*\)</a><table.*|\2\
\1\
|p' | sed 's/<[^>]*>//g'
}
...
URL2='http://cougar.eb.com/soundc11'
...
local INI='^\(Não \)\{0,1\}[Ff]oi\{0,1\}\(ram\)\{0,1\} encontrad' FIM='^Imprimir *$'
...
local D ; for D in babelfish babylon jargon portugues
