...
# 20050630 ++ locale: busca os códigos de locale, tipo pt_BR (valeu kojima)
...
local URL d n=5 CNC9 CNC10 MK92 MK10
local limite="sed ${n}q"; [ "$1" ] && d="$*" ||
  d='conectiva debian slackware gentoo mandrake suse fedora freebsd' 
...
s/\([-0-9]* \) *[a-zA-Z]* *\(.*[^ ]\)  *[0-9][0-9]* *$/\1\2/;p;}' |
$limite ; }
[ "$d" != "${d#*mandrake}" ] && { MK92='9.2'; MK10='10.0'
URL='http://www.mandrakesoft.com/security/advisories?dis='
echo; echozz "** Atualizações Mandrake 9.2" ; echo "$URL$MK92"
$ZZWWWDUMP "$URL$MK92" | sed -n '/Date *Advisory/,${s/^.*.jpg\] *//;
s/^\([-0-9]\{1,\} \) *[-:A-Z0-9]* *\(.*\)/\1  \2/p;}' | $limite
echo; echozz "** Atualizações Mandrake 10.0"; echo "$URL$MK10"
$ZZWWWDUMP "$URL$MK10" | sed -n '/Date *Advisory/,${s/^.*.jpg\] *//;
s/^\([-0-9]\{1,\} \) *[-:A-Z0-9]* *\(.*\)/\1  \2/p;}' | $limite ; }
...
local URL='http://www.academia.org.br/vocabulario/volta_portal_org.asp'
