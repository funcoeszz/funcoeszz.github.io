...
#          <> noticiaslinux: .BR agora é P), LinuxinBrazil agora é B)r Linux
#          <> loteria: lotomania: todas as dezenas (valeu bianchi, hoffmann)
#          <> dicas-l: extensão mudou de .shtml para .php
#          <> arrumado erro ao carregá-las no bashrc (valeu hildebrando)
# 20041111 ++ nomefoto
#          <> arrumado de vez o erro no carregamento (valeu carvalhaes)
#          <> se o script for chamado sem argumentos, não mostra nada
#          <> noticiaslinux: B) URL e filtro atualizados
#          <> noticiassec: C) URL atualizada
#          <> loteria: adicionada lotofácil (valeu casali)
#          <> tv: URL e filtro atualizados (valeu broonu)
#          <> diffpalavra: opções cat/diff/rm, funciona BSD (valeu carvalhaes)
#          <> horacerta: URL e filtro atualizados
# 20041223 ++ natal, security, cpf, cnpj, linha
#          -- dicdict: removida, está quebrada e tem pouco uso
#          <> dicabl: URL e filtro atualizados (valeu sartini)
#          <> ramones: não estava achando certas palavras (valeu rodarvus)
#          <> data: checagem da data passada pelo usuário (valeu rbp)
...
#          <> bovespa: agora com mercado fracionário também (valeu casali)
...
#          <> dicbabelfish: colocadas todas as línguas no --help
# 20050519 ++ novo esquema de funções extras: $ZZEXTRA $ZZEXTRA_DFT ~/.zzextra
#          <> zzzz, ajuda: levando em conta funções extras também
...
#          <> dicportugues: melhorada a formatação da saída
#          <> dicbabylon: melhorada a formatação da saída
...
#          <> whoisbr: agora mostra informações sobre o DNS do domínio
#          <> dolar: filtro atualizado
#          <> linuxnews: S) filtro atualizado
#          <> dicabl: URL atualizada (valeu sartini)
#    !!    ** domínio funcoeszz.net e camiseta 5 anos
#
##############################################################################
#
#                                Configuração
...
cat $ZZPATH $ZZEXTRA | sed '1{s/.*/** Ajuda das funções ZZ (tecla Q sai)/;G;p;}
  /^# --*$/,/^# --*$/{s/-\{20,\}/-------/;s/^# //p;};d' | uniq |
...
if [ "$1" = '--atualiza' ]; then # obtém versão nova, se !=, download
  ...
  vr=$($ZZWWWDUMP $URL | sed -n 's/.*versão atual \([0-9.]\{1,\}\).*/\1/p')
  echo "versão local : $vl"; echo "versão remota: $vr"; echo
  [ "$vr" ] || return ; if [ "$vl" = "$vr" ]; then
    echo 'Você já está com a última versão.'
  else
  ...
  local arq extra; for arq in "$ZZPATH" "$ZZEXTRA"; do
    [ "$arq" -a -f "$arq" ] && {
      echo; echozz "(( funções disponíveis ${extra:+EXTRA }))"
...
# ----------------------------------------------------------------------------
# Mata os processos que tenham o(s) padrão(ões) especificado(s)
...
# juntamente com os respectivos códigos ANSI para obtê-las
# Uso: zzcores
# ----------------------------------------------------------------------------
...
# Uso: zzlinha [número | -t texto] [arquivo]
...
for d in $d1 $d2; do NUM=0 ; if [ "$d" = 'hoje' -o "$d" = 'today' ]; then
 ...
 elif [ "${d##*[^0-9/]}" != "$d" ]; then echo "data inválida '$d'"; return; fi
...
# Uso: zzconverte <cf|fc|ck|kc|fk|kf|mk|km|db|bd|cd> número
# Ex.: zzconverte cf 5
#      zzconverte dc 65
#      zzconverte db 32
# ----------------------------------------------------------------------------
...
elif [ "${cpf#?????????}" = "$d1$d2" ]; then echo CPF válido
else echo "CPF inválido (-$d1$d2)"
fi
}
...
then [ ${#cnpj} -ne 14 ] && { echo "CNPJ inválido"; return; }; base=${cnpj%??}
...
elif [ "${cnpj#????????????}" = "$d1$d2" ]; then echo CNPJ válido
else echo "CNPJ inválido (-$d1$d2)"
fi
}


# ----------------------------------------------------------------------------
# Calcula o endereço da rede e o endereço de broadcast a partir de um 
# ip e uma máscara de rede
...
{ echo IP inválido; return; }; [ "$1" = "${1#*/}" ] && mask=${2:-24} ||
...
sed 's/^ *//;/Data:/,/Turismo/!d;/percent/d;s/  */ /g
     ...
     s@^\([0-9][0-9]\)/\([0-9]/\)@\1/0\2@
     s/^D.lar \|- Corretora\| SP//g;s/ [-+]\{0,1\}[0-9.,]\{1,\}  *%$//
...
[ "$1" ] && patt=$(echo $* | sed 's/ /\\|/g')
$ZZWWWDUMP "$URL/$extra" | sed "/[0-9][0-9]$/!d; /Dólar \(Paral\|Ptax\|Turi\)/d
...
$ZZWWWDUMP "$URL$1" | sed '/Ação\|Fracionário/,/Oferta/!d; //d; /\.gif/d
s/^ *//; /Milhares/q'
}
...
# Procura no Wikipédia (enciclopédia livre)
# Obs.: se nenhum idioma for especificado é utilizado o inglês como padrão
#
# Idiomas: de (alemão)  , eo (esperanto), es (espanhol), fr (frances),
...
local tipo linhas URL=http://www1.caixa.gov.br/loterias/resultados/asp
for tipo in quina megasena duplasena lotomania lotofacil; do echozz $tipo:
 linhas=1; [ "$tipo" = lotomania ] && linhas=2
 [ "$tipo" = lotofacil ] && linhas=3
 lynx -dump -nolist "$URL/$tipo.asp?perfil=loteria" | sed -n '
 /\([0-9][0-9] \{1,3\}- \{1,3\}\)\{4\}/p; /Concurso n/p; /ACUMULOU!/{
 N; s/!.*em / - /p; }' | sed -e ':a' -e '$!N;s/\n/@@/;ta' -e 's/n.mero //
 s/^\( *Con.*)\)@@\(.*\)/\2@@\1/; s/@@\( *ACUMU.*\)@@\(.*\)/@@\2@@\1/
 s/@@ *ACUM/    ACUM/; s/\.\([0-9]\)00\.000,00\..*/.\1 Mi/; s/00,00\..*/00,00/
 s/^\(\([0-9 -]*@@\)\{'$linhas'\}\)  *[0-9].*@@/\1/; s/   *-   */ - /g; s/@@/\
/g' ; done
}
...
[ "$1" ] || { echo 'uso: zzlocale código|texto'; return; }
local txt="$1" URL=http://www.mpi-sb.mpg.de/~pesca/locales.html
[ "$1" = '-c' ] && { txt="$2.*   [^ ]"; }
$ZZWWWDUMP "$URL" | sed '1,2d;$d' | grep -i -- "$txt"
}
...
sed -n 's/\.php/&  /;s@^<LI><A HREF=\([^>]*\)> *\([^ ].*\)</A>@\1: \2@p'
}
...
# Mostra o IP (válido) que você está conectado na Internet
# Uso: zzipinternet
# Ex.: zzipinternet
# ----------------------------------------------------------------------------
zzipinternet(){ zzzz -z $1 zzipinternet && return
local URL='http://www.whatismyip.com'
$ZZWWWDUMP $URL | sed -n 's/^ *Your IP \(Is\|-\)/IP: /p'
}
...
[ "$s" != "${s#*v}" ] && { URL='http://www.vivaolinux.com.br'
  echo ; echozz "* Viva o Linux ($URL):"; $ZZWWWDUMP $URL/index.rdf |
  sed -n '1,/<\/channel>/d;/<title>/s/<[^>]*>//gp' | $limite; }
...
[ "$s" != "${s#*i}" ] && { URL='http://info.abril.com.br/aberto/infonews/'
  echo ; echozz "* InfoExame ($URL):"; $ZZWWWHTML $URL |
  sed -n '/class="teste"/s/<[^>]*>//pg' | $limite; }
[ "$s" != "${s#*u}" ] && { URL='http://www.underlinux.com.br'
  echo ; echozz "* UnderLinux ($URL):"; $ZZWWWDUMP "$URL/backend.php" |
  sed -n '/<item>/{ n; /<title>/s/<[^>]*>//gp; }' | $limite; }
[ "$s" != "${s#*n}" ] && { URL='http://www.noticiaslinux.com.br'
  echo ; echozz "* Notícias Linux ($URL):"; $ZZWWWHTML "$URL" |
  sed -n '/<[hH]3>/{s/<[^>]*>//g;s/^[ \t]*//g;p}' | $limite; }
}
...
[ "$s" != "${s#*s}" ] && { URL='http://slashdot.org'
  ...
  sed -n '/<div class="title">/,/<\/div>/s/.*slashdot.*<\/a> *: *//p'|$limite;}
...
# ----------------------------------------------------------------------------
# http://... - vários
# Mostra os últimos 5 avisos de segurança de várias distribuições e SO
# As suportadas: 
...
echo ; echozz "** Atualizações Conectiva 9"; echo "$URL$CNC9"
$ZZWWWDUMP "$URL$CNC9" |
sed '/^ *200[0-9]-[0-1][0-9]-[0-3][0-9]/!d;s/^ *//' | $limite 
echo ; echozz "** Atualizações Conectiva 10"; echo "$URL$CNC10"
$ZZWWWDUMP "$URL$CNC10" |
sed '/^ *200[0-9]-[0-1][0-9]-[0-3][0-9]/!d;s/^ *//' | $limite ;}
[ "$d" != "${d#*debian}" ] &&  { URL='http://www.debian.org'
echo; echozz "** Atualizações Debian woody"; echo "$URL"
$ZZWWWDUMP "$URL" | 
sed -n '/Security Advisories/,/_______/{/\[[0-9]/s/^ *//p;}' | $limite ;}
[ "$d" != "${d#*slackware}" ] && { echo; echozz "\n** Atualizações Slackware"
...
[ "$d" != "${d#*gentoo}" ] && { echo; echozz "** Atualizações Gentoo"
...
[ "$d" != "${d#*mandriva}" ] && { URL='http://www.mandriva.com/security/rss'
echo; echozz "** Atualizações Mandriva" ; echo "$URL"; $ZZWWWHTML "$URL" |
sed -n '/<title>/{s/<[^>]*>//g;s/^ *//;/MDKSA/p;}' | $limite; }
[ "$d" != "${d#*suse}" ] && { echo; echozz "** Atualizações Suse"
URL='http://www.novell.com/linux/security/advisories.html'; echo "$URL"
$ZZWWWDUMP "$URL"| sed -n '1,/Advisories/d;s/^ *\([0-9][0-9]\)/\1/p'|$limite; }
[ "$d" != "${d#*fedora}" ] && { echo; echozz "** Atualizações Fedora"
URL='http://www.linuxsecurity.com/content/blogcategory/89/102/'; echo "$URL"
$ZZWWWDUMP "$URL"| sed -n 's/^ *\([Ff]edora [Cc]ore.*:.*\) *$/\1/p' |$limite; }
[ "$d" != "${d#*freebsd}" ] && { echo; echozz "** Atualizações FreeBSD"
...
s/.*href=\"[^\"]*\/\([^\"]*\)\".*/\1/p;}'); [ "$a" ] || {
 echo "Programação de hoje não disponivel"; return; }; $ZZWWWDUMP $URL/$a |
...
local URL='http://www.academia.org.br/vocabulario/volta_voca_org.asp'
echo "palavra=$*" | $ZZWWWPOST $URL | sed '1,/:$/d; /)$/d ; s/^ */  /' |
sed '/^ *$/d'
}
...
local INI='^\(Não \)\{0,1\}[Ff]o\(i\|ram\) encontrad' FIM='^Imprimir *$'
...
"
		...
		func="zz${1#zz}"
		if type $func >&- 2>&- ; then   # a função existe?
			shift ; $func "$@"            # sim, vai!
		else
			echo "ERRO: função inexistente '$func' (tente --help)"
		fi
	fi
fi
