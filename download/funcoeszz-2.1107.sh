...
# VEJA TAMBÉM: o conjunto de funções do thobias, similar às funçoes ZZ
...
[ "$s" != "${s#*f}" ] && { URL='http://freshmeat.net'
  ...
  sed '/href="\/releases/!d;s/<[^>]*>//g;s/&nbsp;//g' | $limite ; }
...
[ "$s" != "${s#*d}" ] && { URL='http://www.linuxdailynews.com'
  echo ; echozz "* Linux Daily News ($URL):"; $ZZWWWDUMP $URL |
  sed 's/^ *//;/^Linux and/,/^More\.\.\./!d'|sed '1d;/^\[/d;/^$/d'| $limite ;}
...
[ -f "$tmpwav" ] || {
  arq=`$ZZWWWHTML "$URL?va=$1" | sed "/audio\.pl/!d;s/[^?]*?//;s/=$1.*//p;d"`
  [ "$arq" ] || { echo "$1: palavra não encontrada"; return; }
  dir=`echo $arq | sed 's/^\(.\).*/\1/'`
  $ZZWWWHTML "$URL2/$dir/$arq" > $tmpwav
  echo "Gravado o arquivo '$tmpwav'" ; }
play $tmpwav
}
