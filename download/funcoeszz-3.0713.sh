...
local z=$1 URL='http://www.tldp.org/HOWTO/' arq=$ZZTMP.howto
[ "$z" = '--atualiza' ] && { rm -f $arq ; z='' ; }
[ -s "$arq" ] || { echo -n 'AGUARDE. Atualizando listagem...'
  $ZZWWWHTML "$URL/HOWTO-INDEX/howtos.html" | sed '/^HREF="..\//!d;s///;
  s/"$//;s,/index.html$,,;s,^mini/\(.*\),\1#,' > $arq ; echo ' feito!' ; }
[ "$z" ] && grep -i "$z" $arq | sed "s,\(.*\)#$,mini/\1,;s,^,$URL,"
}
...
#      zznoticiassec bcf
# ----------------------------------------------------------------------------
zznoticiassec(){ zzzz -z $1 zzsec && return
