...
local pat rc cor='não'; [ "$ZZC" = '1' ] && cor='sim' ; [ -f "$ZZPATH" ] &&
...
if [ "$1" = '-z' -o "$1" = '-h' -o "$1" = '--help' ]; then # -h)zzzz -z)resto
  ...
  :z;s/.*\n---*\(\n\)\(.*$pat\)/\1\2/;s/\(\n\)---.*/\1/"; return 0
elif [ "$1" = '--atualiza' ]; then # obtém versão nova, se !=, download 
  echo "versão local : $vl"; echo -n "versão remota: " # download hp & limpa
  ...
  if [ "$vl" = "$vr" ]; then echo '  você já está com a última versão.'
    ...
    echo '  baixando a versão nova...'; local urlrpm rpm exe;
    urlrpm=`sed '/^Pacot/!d;s/.* //' $ZZTMP`; rpm="${urlrpm##*/}"
    urlexe=`sed '/^Arqui/!d;s/.* //' $ZZTMP`; exe="${urlexe##*/}-$vr"
    $ZZWWWHTML $urlexe > $exe; echo "@ executável: $exe"
    $ZZWWWHTML $urlrpm > $rpm; echo "@ pacote RPM: $rpm"
  ...
  sed '/^zz[a-z0-9]\+(/!d;s/^zz//;s/(.*//' $ZZPATH | sort | pr -l 10 -w 72 -5
  }
fi
}
...
[ "$s" != "${s#*t}" ] && { URL='http://www.tchelinux.com.br'
  echozz "* TcheLinux ($URL):"; $ZZWWWDUMP "$URL/backend.php" |
...
# ----------------------------------------------------------------------------
zzcasadosartistas(){ zzzz -z $1 zzcasadosartistas && return
...
local URL arq dir tmpwav URL2='http://cheetah.eb.com/sound'
URL='http://www.m-w.com/cgi-bin/dictionary'
arq=`$ZZWWWHTML "$URL?va=$1" | sed '/audio\.pl/!d;s/[^?]*?//;s/=.*//' | sed q`
[ "$arq" ] || { echo "$1: palavra não encontrada"; return; }
dir=`echo $arq | sed 's/^\(.\).*/\1/'` ; tmpwav=$ZZTMP.$arq
[ -f "$tmpwav" ] || { $ZZWWWHTML "$URL2/$dir/$arq" > $tmpwav
echo "Gravado o arquivo '$tmpwav'" ; } ; play $tmpwav
}
...
local from="$1" to="$2" URL=http://page.nextel.com.br/cgi-bin/sendPage_v2.cgi
...
# ----------------------------------------------------------------------------
zzdicmichaelis(){ zzzz -z $1 zzdicmichaelis && return
...
if [ "$1" ]; then
  if [ "$1" = '--help' -o "$1" = '-h' ]; then $0
    ...
    "zz${@#zz}" || echo 'ERRO: esta função não existe! (tente --help)'
  fi
