...
for tipo in quina megasena duplasena lotomania; do echozz $tipo:
 ...
 sed -n '/\([0-9][0-9] - \)\{4\}/p;/Concurso n/p' |
 sed ':a;$!N;s/\n/@@/;ta ; s/n.mero // ; s/^\( *Con.*)\)@@\(.*\)/\2@@\1/;
 s/^\(\([0-9 -]*@@\)\{'$linhas'\}\)  *[0-9].*@@/\1/ ; s/@@/\
/g'; done
}
...
[ "$s" != "${s#*y}" ] && { URL='http://br.news.yahoo.com/tecnologia/linux.html'
  echo ; echozz "* Yahoo Linux ($URL):"; $ZZWWWHTML $URL | tee /tmp/foo | 
...
[ "$s" != "${s#*b}" ] && { URL='http://br-linux.org/noticias/'
...
sed -n '/^ *UTC/,/calculate future/{/^ \{8,\}/{s///;:a;s/^.\{1,60\}$/ & /;ta;}
/^ *\[/d;/calculate future/d;p;}'
}
