...
# AUTOR     : aurélio marinho jargas <verde@verde666.org>
# INSTALAÇÃO: echo source /rota/para/funcoeszz >> ~/.bashrc
#
# DESCRIÇÃO:
...
#
# INFORMAÇÕES:
# acesse a página (( http://verde666.org/zz ))
#
...
# 20010809 ++ irpf (tks stulzer), <> detran -> detranpr
...
# 20011001 ++ chavepgp (tks missiaggia)
# 20011002 <> limpalixo: aceita $1 também, ++ /usr/bin/funcoeszz (tks gwm)
#          <> dolar: URL nova, formato novo (tks bruder)
# 20011015 <> arrumanome: s/^-/_/, mv -v --
# 20011018 <> "$@" na chamada do executável (++aspas) 
# 20011108 <> dolar: formato melhorado
# 20011113 ++ cores
# 20011211 <> freshmeat: mudança no formato (©), ++ detransp (tks elton)
#          ++ $ZZER{DATA,HORA}
# 20011217 ++ noticiaslinux, whoisbr (tks marçal)
...
# números fracionados podem vir com vírgulas ou pontos: 1,5 ou 1.5
...
# busca o CEP de qualquer rua de qualquer cidade do país
# uso: zzcep cidade nome-da-rua
# ex.: zzcep curitiba rio gran
#      zzcep 'Rio de Janeiro' Vinte de
# ----------------------------------------------------------------------------
zzcep(){
[ "$2" ] || { echo 'uso: zzcep cidade nome-da-rua'; return; }
local r c=`echo "$1"| sed "$ZZSEDURL"`; shift; r=`echo "$*"| sed "$ZZSEDURL"`
echo "_textCidade=$c&_textRua=$r&_b_r_c.x=1&_b_r_c.x=0&_b_r_c.y=0" |
$ZZWWWPOST 'http://www.terra.com.br/cep/ceps.cgi' | sed '1,2d;$d;s/^ *//
/^Mapas (opcional):$/{N;N;d;};/^\[USEMAP:modo\.gif\]$/{N;d;};s/^/  /'
}
...
local dump=`echo $ZZWWWDUMP| sed 's/-nolist -crawl//'`
...
local o URL='www.dicas-l.unicamp.br'; [ "${1##-*}" ] || { o=$1; shift; }
echo "-- $URL/dicas-l/<DATA>.shtml"; $ZZWWWHTML "http://$URL/listagem.html" |
...
# uso: zzregistrobr domínio
# ex.: zzregistrobr abc.com.br
#      zzregistrobr www.abc.com.br
# ----------------------------------------------------------------------------
zzwhoisbr(){
[ "$1" ] || { echo 'uso: zzregistrobr domínio'; return; }
...
# siglas: B)and, C)nt, E)ducativa, G)lobo, R)ecord, S)bt, cU)ltura
